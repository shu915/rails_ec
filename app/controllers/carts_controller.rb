# frozen_string_literal: true

class CartsController < ApplicationController
  before_action :setup_cart_item!, only: %i[create destroy]
  def show
    @cart_items = current_cart.cart_items.includes([:product])

    @total = @cart_items.sum do |cart_item|
      if cart_item.product.discouted_price.present?
        cart_item.quantity * cart_item.product.discouted_price
      else
        cart_item.quantity * cart_item.product.price
      end
    end
  end

  def create
    @cart_item ||= current_cart.cart_items.build(product_id: cart_params[:product_id])
    @cart_item.quantity += cart_params[:quantity].to_i
    if @cart_item.save
      flash[:notice] = "#{@cart_item.product.name}が追加されました。"
    else
      flash[:errors] = '商品の追加に失敗しました。'
    end
    redirect_back(fallback_location: root_path)
  end

  def destroy
    @cart_item = CartItem.find(cart_params[:id])
    @cart_item.destroy
    redirect_to cart_path
  end

  private

  def setup_cart_item!
    @cart_item = current_cart.cart_items.find_by(product_id: cart_params[:product_id])
  end

  def cart_params
    params.permit(:id, :product_id, :quantity)
  end
end
